...
//
    ...
    public function register(Request $request)
    {
        ...
        if ($request->profile_img && $request->profile_img != 0) {
    ...
    public function recover(Request $request)
    {
        ...
        Mail::to($user->username)->send(new \App\Mail\MyRecoverMail($details));
    ...
    public function verifyEmail($verify_code)
    {
        ...
        if(strpos(URL::to('auth/login?status=1'), '-api') != false) {
            $url = str_replace('-api.', '.', URL::to('auth/login?status=1'));
        }else {
            $url = str_replace('api.', '', URL::to('auth/login?status=1'));
        }
